  ...
  useEffect(() => {
    ...
    gapiScript.onload = () => {
      (window as any).gapi.load('client', () => {
        console.log('AuthContext: Initializing GAPI with API Key:', process.env.API_KEY); // Added for debugging
        (window as any).gapi.client
          .init({
            apiKey: process.env.API_KEY, // Use process.env.API_KEY
    ...
    gisScript.onload = () => {
      try {
        console.log('AuthContext: Initializing GIS with Client ID:', process.env.GOOGLE_CLIENT_ID); // Added for debugging
        const client = (window as any).google.accounts.oauth2.initTokenClient({
          client_id: process.env.GOOGLE_CLIENT_ID, // Use process.env.GOOGLE_CLIENT_ID
          scope: SCOPES,
