...
export async function getHeaderMap(
  ...
  try {
    ...
    throw new Error(
      `Could not fetch headers for sheet: ${sheetName}. Check sheet name and permissions.`,
    );
  }
}

async function batchGetHeaderMaps(
  spreadsheetId: string,
  sheetNames: string[],
): Promise<void> {
  ...
  try {
    ...
    console.error(
      `Error batch fetching headers for spreadsheet '${spreadsheetId}':`,
      err,
    );
...
>(
  ...
  for (const chunk of configChunks) {
    ...
    const sheetNamesInChunk = [
      ...new Set(chunk.map((c) => c.config.range.split('!')[0])),
    ];
    ...
    valueRanges.forEach((valueRange: any, index: number) => {
      ...
      if (!headerMap || Object.keys(headerMap).length === 0) {
        console.warn(
          `No headers found for sheet '${sheetName}'. Returning empty array for key '${key}'.`,
        );
        allResults[key] = [];
        return;
      }
      ...
      const parsedData = dataRows
        .map((row: any[]) => {
          ...
          for (const itemKey in columns) {
            ...
            const rawValue: any =
              colIndex !== undefined && row[colIndex] !== null
                ? row[colIndex]
                : undefined;
        ...
        .filter(
          (item: T) => item && item[keyField] && String(item[keyField]).trim() !== '',
        );
...
async function findRowIndex<T>(config: SheetConfig<T>, id: string): Promise<number> {
  ...
  if (!keyColumnHeader)
    throw new Error(
      `Key field header for '${String(keyField)}' not found in config.`,
    );

  const keyColumnIndex = headerMap[keyColumnHeader];
  if (keyColumnIndex === undefined)
    throw new Error(`Column '${keyColumnHeader}' not found in sheet '${sheetName}'.`);
...
export function mapEntityToRowArray<T>(
  entity: Partial<T>,
  ...
  const totalColumns =
    Object.keys(headerMap).length > 0
      ? Math.max(...Object.values(headerMap)) + 1
      : 0;
...
export async function createEntity<T extends { [key: string]: any }>(
...
export async function appendEntity<T>(
  config: SheetConfig<T>,
  entityData: T,
): Promise<void> {
...
export async function updateEntity<T extends { [key: string]: any }>(
  config: SheetConfig<T>,
  entityData: T,
): Promise<T> {
  const { spreadsheetId, range, columns, keyField } = config;
