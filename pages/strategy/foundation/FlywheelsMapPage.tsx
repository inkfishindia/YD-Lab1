    ...
    return (
        <div className="p-4 space-y-4">
            ...
            <DetailSection title="Target Audience Deep Dive">
                {(segment['age group']) && <p><strong>Age Group:</strong> {segment['age group']}</p>}
            ...
            <DetailSection title="Platforms">
                <div className="flex flex-wrap gap-2">
                    {relatedPlatforms.map(platform => (
                        ...
                        >
                            {platform.platform_icon_url && <img src={platform.platform_icon_url} alt={platform.platform_name} className="w-5 h-5 rounded-sm object-contain" />}
...
const BuDetailView: React.FC<{ item: SystemBusinessUnit; onSelect: (type: SystemEntityType, id: string) => void; }> = ({ item, onSelect }) => {
    ...
    const servesSegment = systemSegments.find(s => item.serves_segments_ids?.includes(s.segment_id));

    return (
        <div className="p-4 space-y-4">
            <h3 className="text-xl font-bold text-white">{item.bu_name}</h3>
            {owner && <p className="text-sm text-gray-400">Owner: {owner.full_name}</p>}
            <p className="text-sm text-gray-400">Status: <span className="font-semibold text-gray-300">{item.status}</span></p>
...
const DetailSidebar: React.FC<{ selection: { type: SystemEntityType; id: string } | null; onClose: () => void; onSelect: (type: SystemEntityType, id: string) => void }> = ({ selection, onClose, onSelect }) => {
    ...
    const renderContent = () => {
        ...
        switch (type) {
            ...
            case 'person': const person = data.systemPeople.find(i => i.user_id === id); return person && <PersonDetailView item={person} onSelect={onSelect} />;
...
const FlywheelsMapPage: React.FC = () => {
    ...
    const highlightedIds = useMemo(() => {
        ...
        while(changed) {
            ...
            data.systemPeople.forEach(p => { if (sets.person.has(p.user_id)) { if(p.primary_hub) sets.hub.add(p.primary_hub); p.owns_flywheels_ids?.forEach(id => sets.flywheel.add(id)); p.owns_segments_ids?.forEach(id => sets.segment.add(id)); p.owns_bus_ids?.forEach(id => sets.bu.add(id)); }});
                ...
                <section>
                    ...
                    <div className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] gap-4">
                        {data.systemBusinessUnits.map(bu => {
                            ...
                            const statusColor = bu.status ? SYSTEM_STATUS_COLORS[bu.status] : 'bg-gray-600';
