...
// --- TYPE DEFINITIONS ---
type ItemType = 'program' | 'project' | 'milestone' | 'task' | 'person' | 'segment' | 'platform' | 'channel' | 'hub';
type StructuredProject = MgmtProject & { milestones: (Milestone & { tasks: MgmtTask[] })[] };
...
const formatNumber = (num: number | undefined | null, style: 'currency' | 'percent' | 'decimal' = 'decimal'): string => {
    ...
    <div className="flex gap-3 overflow-x-auto pb-3 -mx-1 px-1">
        {segments.map(segment => {
            ...
            const relatedPlatforms = (() => {
                ...
                const platformIds = segment.Platforms.split(/[,|]/).map(id => id.trim());
            ...
            return (
                ...
                >
                    <div className="flex-grow">
                        ...
                        {segment.Promise && <p className="text-sm text-center text-gray-300 italic mt-2">"{segment.Promise}"</p>}
                    </div>
                    ...
                    {relatedPlatforms.length > 0 && (
                        <div className="mt-2 pt-2 border-t border-gray-700/50 flex items-center gap-2.5">
                            {relatedPlatforms.map(platform => (
                                <a key={platform.platform_id} href={platform.platform_link || '#'} target="_blank" rel="noopener noreferrer" title={platform.platform_name} className="block" onClick={(e) => e.stopPropagation()}>
    ...
    <div className="flex gap-3 overflow-x-auto pt-1 pb-3 -mx-1 px-1">
        {platforms.map(platform => {
            ...
            return (
                ...
                >
                    {platform.platform_icon && <img src={platform.platform_icon} alt="" className="w-5 h-5 rounded-sm object-contain" />}
...
}> = ({ hubs, people, selection, onSelect, isFiltered, highlightedHubIds, highlightedPersonIds }) => {
    
    const hubOrder = ["Digital Platform", "Production & Fulfillment", "Sales & Business Dev", "Marketing & Growth"];
    const sortedHubs = useMemo(() => 
        [...hubs].sort((a, b) => {
            const aIndex = hubOrder.indexOf(a.hub_name);
            const bIndex = hubOrder.indexOf(b.hub_name);
            if (aIndex === -1 && bIndex === -1) return a.hub_name.localeCompare(b.hub_name);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 auto-rows-min">
            {sortedHubs.map(hub => {
                ...
                return (
                    <div key={hub.hub_id} className={`bg-gray-900 border rounded-lg flex flex-col transition-opacity duration-300 ${isHubDimmed ? 'opacity-30' : ''} ${isHubSelected ? 'border-accent-green' : 'border-gray-800'}`}>
                        {/* Hub Header */}
                        <div onClick={() => onSelect('hub', hub.hub_id)} className="p-4 bg-gray-800/40 rounded-t-lg border-b border-gray-800 cursor-pointer group relative">
                            <h3 className="font-bold text-lg text-accent-green flex items-center gap-2"><Squares2X2Icon className="w-5 h-5"/>{hub.hub_name}</h3>
                        ...
                        <div className="p-2 grid grid-cols-2 gap-1 flex-1 items-start content-start">
                            {relatedPeople.map(person => {
                                const isPersonSelected = selection?.type === 'person' && selection.id === person.person_id;
                                const isPersonHighlighted = highlightedPersonIds.has(person.person_id);
                                const isPersonDimmed = isFiltered && !isPersonHighlighted;

                                return (
                                    <div key={person.person_id} onClick={(e) => { e.stopPropagation(); onSelect('person', person.person_id); }} className={`p-2 rounded-md cursor-pointer transition-colors ${isPersonDimmed ? 'opacity-30' : ''} ${isPersonSelected ? 'bg-blue-900/50' : 'hover:bg-gray-800'}`}>
...
const ProjectCard: React.FC<{
    ...
    const tooltipContent = (
        <div className="text-xs max-w-xs">
            ...
            {project.objective && <div><p className="font-semibold text-gray-400">Objective:</p><p>{project.objective}</p></div>}
            {project.success_metric && <div className="mt-2"><p className="font-semibold text-gray-400">Success Metric:</p><p>{project.success_metric}</p></div>}
        </div>
    );
    ...
    return (
        <div 
            ...
            onMouseLeave={onClearTooltip}
            className={`p-3 rounded-lg cursor-pointer transition-all duration-300 bg-gray-800 hover:bg-gray-700 border border-gray-700/50 ${isDimmed ? 'opacity-30' : ''}`}
        >
            <div className="flex justify-between items-start">
                <h4 className="font-semibold text-white truncate pr-2 text-sm">{project.project_name}</h4>
                <Badge text={project.status} colorClass={STATUS_COLORS[project.status as keyof typeof STATUS_COLORS]} size="sm"/>
            </div>
            <div className="flex justify-between items-center mt-1 text-xs text-gray-400">
                <span>{project.owner_name}</span>
                <span>{project.end_date}</span>
            </div>
            <div className="mt-2 text-xs text-gray-500">{totalTasks} Tasks ({completedTasks} Done)</div>
...
const ProgramColumn: React.FC<{
    ...
    onTooltip: (e: React.MouseEvent, content: React.ReactNode) => void,
    onClearTooltip: () => void,
}> = ({ program, isDimmed, highlightedProjIds, onCardClick, onTooltip, onClearTooltip }) => {
    const programTooltipContent = (
        <div className="text-xs max-w-xs">
            <p className="font-bold text-base text-white mb-2">{program.program_name}</p>
            {program.customer_problem && <div><p className="font-semibold text-gray-400">Problem:</p><p>{program.customer_problem}</p></div>}
            {program.our_solution && <div className="mt-2"><p className="font-semibold text-gray-400">Solution:</p><p>{program.our_solution}</p></div>}
            {program.blockers && <div className="mt-2"><p className="font-semibold text-red-400">Blockers:</p><p className="text-red-300">{program.blockers}</p></div>}
        </div>
    );

    return (
        <div className="bg-gray-900 border border-gray-800 rounded-xl w-[340px] flex-shrink-0 h-full flex flex-col">
            <div 
                className={`p-3 border-b border-gray-800 flex-shrink-0 cursor-pointer transition-opacity duration-300 ${isDimmed ? 'opacity-50' : ''}`}
                onClick={() => onCardClick('program', program)}
                onMouseEnter={(e) => onTooltip(e, programTooltipContent)}
                onMouseLeave={onClearTooltip}
            >
                <div className="flex justify-between items-start">
                    <h3 className="text-base font-semibold text-white">{program.program_name}</h3>
                    <Badge text={program.priority || ''} colorClass={PRIORITY_COLORS[program.priority as Priority]} size="sm"/>
                </div>
                <p className="text-xs text-gray-400 mt-1">{program.owner_person_name}</p>
                
                <div className="mt-3 space-y-1.5">
                    <div>
                        <div className="text-xs text-gray-500 flex justify-between"><span>Timeline</span><span>{program.days_remaining} days left</span></div>
                        <div className="w-full bg-gray-700 rounded-full h-2">
                            <div className={`${getHealthColor(program.health_status)} h-2 rounded-full`} style={{ width: `${program.timeline_progress_pct || 0}%` }}></div>
                        </div>
                    </div>
                     <div>
                        <div className="text-xs text-gray-500 flex justify-between"><span>{program.success_metric}</span><span>{formatNumber(program.current_value)}/{formatNumber(program.target_value)}</span></div>
                        <div className="w-full bg-gray-700 rounded-full h-2">
                            <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${program.metric_progress_pct || 0}%` }}></div>
                        </div>
                    </div>
                </div>
            </div>
...
const TaskCard: React.FC<{ task: MgmtTask; onClick: () => void; isDimmed: boolean }> = ({ task, onClick, isDimmed }) => (
    <div onClick={onClick} className={`p-3 bg-gray-800 rounded-lg border border-gray-700/50 cursor-pointer hover:bg-gray-700 transition-opacity duration-300 ${isDimmed ? 'opacity-30' : ''}`}>
        <p className="font-medium text-white text-sm">{task.task_name}</p>
        <div className="flex justify-between items-center mt-2 text-xs">
            <Badge text={task.status} colorClass={STATUS_COLORS[task.status as keyof typeof STATUS_COLORS]} size="sm"/>
...
const TaskList: React.FC<{ tasks: MgmtTask[]; onCardClick: (type: ItemType, item: MgmtTask) => void, showPrompt: boolean; isFiltered: boolean; highlightedTaskIds: Set<string> }> = ({ tasks, onCardClick, showPrompt, isFiltered, highlightedTaskIds }) => (
    <div className="flex flex-col bg-gray-900 border border-gray-800 rounded-xl h-full">
        <div className="p-3 border-b border-gray-800">
            <h2 className="text-lg font-semibold text-white flex items-center gap-2"><ClipboardCheckIcon className="w-5 h-5" /> Tasks</h2>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {tasks.length > 0 ? (
                tasks.map(task => {
                    const isDimmed = isFiltered && !highlightedTaskIds.has(task.task_id);
                    return <TaskCard key={task.task_id} task={task} onClick={() => onCardClick('task', task)} isDimmed={isDimmed} />
                })
            ) : (
                showPrompt && <div className="text-center text-gray-500 p-8 h-full flex items-center justify-center">Select an item to see related tasks.</div>
            )}
        </div>
    </div>
);
...
const DetailContent: React.FC<{ type: ItemType | null, item: any | null, onSelect: (type: ItemType, id: string) => void }> = ({ type, item, onSelect }) => {
    ...
    const renderField = (label: string, value: any, className?: string) => value ? <p className={className}><strong className="text-gray-400 w-28 inline-block">{label}:</strong> {String(value)}</p> : null;
    
    const DetailSection: React.FC<{title: string, children: React.ReactNode}> = ({title, children}) => {
        const hasContent = React.Children.toArray(children).some(child => child || child === 0);
    ...
    const RelatedItem: React.FC<{icon: React.ElementType, name: string, detail: string, onClick: () => void}> = ({icon: Icon, name, detail, onClick}) => (
    ...
    switch (type) {
        case 'program': {
            ...
            const relatedProjects = mgmtProjects.filter(p => p.program_id === prog.program_id);
            return (
                <div className="space-y-2 text-sm text-gray-300">
                    ...
                    <DetailSection title="Key Info">
                     ...
                     <DetailSection title="Financials">
                        ...
                        {renderField('Burn Rate', formatNumber(prog.budget_burn_rate_pct, 'percent'))}
                    </DetailSection>
                    <DetailSection title="Health & Status">
                        {renderField('Health', prog.health_status, `font-semibold ${getHealthColor(prog.health_status)?.replace('bg-', 'text-')}`)}
                        {renderField('Risk Level', prog.risk_level)}
                        {renderField('Blockers', prog.blockers, 'text-red-400')}
                        {renderField('Next Milestone', `${prog.next_milestone} (${prog.next_milestone_date})`)}
                    </DetailSection>
                    <DetailSection title="Projects">
                        {relatedProjects.map(p => <RelatedItem key={p.project_id} icon={FolderIcon} name={p.project_name} detail={p.status} onClick={() => onSelect('project', p.project_id)}/>)}
                    </DetailSection>
                </div>
            );
        }
        case 'project': {
            ...
            const relatedMilestones = milestones.filter(m => m.project_id === proj.project_id);
            return (
                <div className="space-y-2 text-sm text-gray-300">
                    ...
                    <DetailSection title="Financials">
                     ...
                     <DetailSection title="Progress">
                        ...
                        {renderField('Velocity', `${proj.velocity_tasks_per_day?.toFixed(2)} tasks/day`)}
                    </DetailSection>
                    <DetailSection title="Milestones">
                        {relatedMilestones.map(m => <RelatedItem key={m.milestone_id} icon={TargetIcon} name={m.milestone_name} detail={m.status} onClick={() => onSelect('milestone', m.milestone_id)}/>)}
                    </DetailSection>
                </div>
            );
        }
        case 'milestone': {
            ...
            const relatedTasks = mgmtTasks.filter(t => t.milestone_id === mile.milestone_id);
            return (
                <div className="space-y-2 text-sm text-gray-300">
                    ...
                    <DetailSection title="Tasks">
                        {relatedTasks.map(t => <RelatedItem key={t.task_id} icon={ClipboardCheckIcon} name={t.task_name} detail={t.status} onClick={() => onSelect('task', t.task_id)}/>)}
                    </DetailSection>
                </div>
            )
        }
        ...
        case 'person': {
         ...
         case 'segment': {
            ...
            return (
                 <div className="space-y-2 text-sm text-gray-300">
                    <h3 className="text-xl font-bold text-white mb-2">{segment.segment_name}</h3>
                    {renderField('Promise', segment.Promise)}
                    {renderField('Vision', segment.vision)}
                    <DetailSection title="Customer Profile">{segment.customer_profile}</DetailSection>
                </div>
            )
        }
        case 'platform': {
            const platform = item as SystemPlatform;
            const platformOwnerHub = systemHubs.find(h => h.hub_id === platform.owner_hub);
            return (
                <div className="space-y-2 text-sm text-gray-300">
                    <h3 className="text-xl font-bold text-white mb-2">{platform.platform_name}</h3>
                    {renderField('Type', platform.platform_type)}
                    {platformOwnerHub && 
                        <DetailSection title="Owner Hub">
                           <RelatedItem icon={Squares2X2Icon} name={platformOwnerHub.hub_name} detail={platformOwnerHub.hub_type} onClick={() => onSelect('hub', platformOwnerHub.hub_id)} />
                        </DetailSection>
                    }
                </div>
            );
        ...
        case 'channel': {
            ...
            const channelOwner = systemPeople.find(p => p.person_id === channel.responsible_person);
            return (
                <div className="space-y-2 text-sm text-gray-300">
                    <h3 className="text-xl font-bold text-white mb-2">{channel.channel_name}</h3>
        ...
        case 'hub': {
            ...
            return (
                <div className="space-y-2 text-sm text-gray-300">
                    ...
                    <DetailSection title="Core Capabilities">{hub.core_capabilities}</DetailSection>
                </div>
            )
        }
        default:
            return <pre className="text-xs text-gray-300 whitespace-pre-wrap">{JSON.stringify(item, null, 2)}</pre>;
    }
};
...
const DetailPane: React.FC<{
    ...
    <div className={`fixed top-16 right-0 h-[calc(100%-4rem)] bg-gray-950 border-l border-gray-800 shadow-2xl z-40 transition-transform duration-300 ease-in-out ${detail.isOpen ? 'translate-x-0' : 'translate-x-full'}`} style={{ width: '400px' }}>
        <div className="flex flex-col h-full">
            <div className="p-4 border-b border-gray-800 flex justify-between items-center flex-shrink-0">
                <h2 className="text-lg font-semibold text-white capitalize">{detail.type} Details</h2>
...
const ProgramsViewPage: React.FC = () => {
    const { programs, mgmtProjects, milestones, mgmtTasks, systemPeople, systemSegments, systemPlatforms, systemChannels, systemHubs, loading } = useData();

    const [selection, setSelection] = useState<{ type: ItemType; id: string } | null>(null);
    const [detailPane, setDetailPane] = useState<{ isOpen: boolean; type: ItemType | null; item: any | null }>({ isOpen: false, type: null, item: null });
    ...
    const handleSelect = useCallback((type: ItemType, id: string) => {
        const isDeselecting = selection?.type === type && selection.id === id;
        setSelection(isDeselecting ? null : { type, id });
        
        if (isDeselecting) {
             if (detailPane.isOpen && detailPane.type === type && detailPane.item && detailPane.item[`${type}_id`] === id) {
                handleClosePane();
            }
        } else {
            ...
            let itemToShow = null;
            switch(type) {
                case 'segment': itemToShow = systemSegments.find(i => i.segment_id === id); break;
                case 'platform': itemToShow = systemPlatforms.find(i => i.platform_id === id); break;
                case 'channel': itemToShow = systemChannels.find(i => i.channel_id === id); break;
                case 'hub': itemToShow = systemHubs.find(i => i.hub_id === id); break;
                case 'person': itemToShow = systemPeople.find(i => i.person_id === id); break;
            }
            if (itemToShow) {
                setDetailPane({ isOpen: true, type, item: itemToShow });
            }
        }
    }, [selection, detailPane.isOpen, detailPane.type, detailPane.item, handleClosePane, systemSegments, systemPlatforms, systemChannels, systemHubs, systemPeople]);


    const handleCardClick = useCallback((type: ItemType, item: any) => {
        const getId = (itemType: ItemType, itemObj: any) => {
            if (!itemObj) return null;
            const keyMap: Record<ItemType, string> = {
                program: 'program_id', project: 'project_id', milestone: 'milestone_id',
                task: 'task_id', person: 'person_id', segment: 'segment_id',
                platform: 'platform_id', channel: 'channel_id', hub: 'hub_id'
            };
    ...
    const sortedSegments = useMemo(() => [...systemSegments].sort((a, b) => (a.priority_rank || '99').localeCompare(b.priority_rank || '99')), [systemSegments]);
    const sortedPeople = useMemo(() => [...systemPeople].sort((a,b) => a.full_name.localeCompare(b.full_name)), [systemPeople]);
    ...
    const selectionData = useMemo(() => {
        ...
        if (selection) {
            switch (selection.type) {
                case 'segment':
                    programs.forEach(p => { if (p.serves_segment_ids?.includes(selection.id)) highlights.programs.add(p.program_id); });
                    mgmtProjects.forEach(p => { if (p.segment_impact?.includes(selection.id)) highlights.projects.add(p.project_id); });
                    break;
                case 'platform':
                    programs.forEach(p => { if (p.platform_ids?.includes(selection.id)) highlights.programs.add(p.program_id); });
                    mgmtProjects.forEach(p => { if (p.platform_id === selection.id) highlights.projects.add(p.project_id); });
                    break;
                case 'channel':
                    programs.forEach(p => { if (p.channel_ids?.includes(selection.id)) highlights.programs.add(p.program_id); });
                    mgmtProjects.forEach(p => { if (p.channel_ids?.includes(selection.id)) highlights.projects.add(p.project_id); });
                    break;
                case 'hub':
                    highlights.hubs.add(selection.id);
                    programs.forEach(p => { if (p.owner_hub_id === selection.id || p.contributing_hub_ids?.includes(selection.id)) highlights.programs.add(p.program_id); });
                    mgmtProjects.forEach(p => { if (p.hub_id === selection.id) highlights.projects.add(p.project_id); });
                    mgmtTasks.forEach(t => { if (t.hub_id === selection.id) highlights.tasks.add(t.task_id); });
                    systemPeople.forEach(p => { if (p.primary_hub === selection.id) highlights.people.add(p.person_id); });
                    break;
                case 'person':
                    highlights.people.add(selection.id);
                    programs.forEach(p => { if (p.owner_person_id === selection.id) highlights.programs.add(p.program_id); });
                    mgmtProjects.forEach(p => { if (p.owner_id === selection.id) highlights.projects.add(p.project_id); });
                    mgmtTasks.forEach(t => { if (t.owner_id === selection.id || t.assignee_ids?.includes(selection.id)) highlights.tasks.add(t.task_id); });
        ...
        mgmtProjects.forEach(p => { if (p.program_id && highlights.programs.has(p.program_id)) highlights.projects.add(p.project_id); });
        mgmtTasks.forEach(t => { 
          const milestone = milestones.find(m => m.milestone_id === t.milestone_id);
          if ((t.project_id && highlights.projects.has(t.project_id)) || (milestone && milestone.project_id && highlights.projects.has(milestone.project_id))) {
    ...
    }, [selection, programs, mgmtProjects, mgmtTasks, milestones, systemHubs, systemPeople]);
    
    const structuredData: StructuredProgram[] = useMemo(() => {
        const tasksByMilestone = new Map<string, MgmtTask[]>();
        mgmtTasks.forEach(task => { if (task.milestone_id) { if (!tasksByMilestone.has(task.milestone_id)) { tasksByMilestone.set(task.milestone_id, []); } tasksByMilestone.get(task.milestone_id)!.push(task); } });
        const milestonesByProject = new Map<string, (Milestone & { tasks: MgmtTask[] })[]>();
        milestones.forEach(mile => { if (mile.project_id) { if (!milestonesByProject.has(mile.project_id)) { milestonesByProject.set(mile.project_id, []); } milestonesByProject.get(mile.project_id)!.push({ ...mile, tasks: tasksByMilestone.get(mile.milestone_id) || [] }); } });
        const projectsByProgram = new Map<string, StructuredProject[]>();
        mgmtProjects.forEach(proj => { if (proj.program_id) { if (!projectsByProgram.has(proj.program_id)) { projectsByProgram.set(proj.program_id, []); } projectsByProgram.get(proj.program_id)!.push({ ...proj, milestones: milestonesByProject.get(proj.project_id) || [] }); } });
        return programs.map(prog => ({ ...prog, projects: projectsByProgram.get(prog.program_id) || [] }));
    ...
    const displayedTasks = useMemo(() => {
        ...
        if (item) {
            if (type === 'program') {
                const programProjectIds = new Set((item as StructuredProgram).projects.map(p => p.project_id));
                tasksToShow = mgmtTasks.filter(t => t.project_id && programProjectIds.has(t.project_id));
            } else if (type === 'project') {
                tasksToShow = mgmtTasks.filter(t => t.project_id === (item as MgmtProject).project_id);
            } else if (type === 'milestone') {
                tasksToShow = mgmtTasks.filter(t => t.milestone_id === (item as Milestone).milestone_id);
            } else if (type === 'task') {
                tasksToShow = mgmtTasks.filter(t => t.task_id === (item as MgmtTask).task_id);
            }
        } else if (selectionData.isFiltered) {
             tasksToShow = mgmtTasks.filter(t => selectionData.highlights.tasks.has(t.task_id));
        }
        return tasksToShow.sort((a,b) => (a.due_date || 'z').localeCompare(b.due_date || 'z'));
        ...
        <div className="h-full flex flex-col gap-4 overflow-y-auto pr-2 pb-4">
            ...
            <div className="flex-shrink-0">
                <div className="flex justify-between items-center mb-2 px-1">
                    <div className="text-center flex-grow">
                        ...
                        <p className="text-sm text-gray-400 -mt-1">Filter programs and projects by customer segment</p>
                    </div>
                    {selection && (
                        <button onClick={() => { setSelection(null); handleClosePane(); }} className="flex items-center gap-2 text-sm text-gray-400 hover:text-white flex-shrink-0">
            ...
            <div className="flex-1 flex flex-col gap-4 min-h-0">
                ...
                <div className="flex-1 flex gap-4 min-h-0">
                            ...
                            {loading ? <>
                                ...
                                structuredData.map(program => (
                                    <ProgramColumn
                                        ...
                                        program={program}
                                        isDimmed={selectionData.isFiltered && !selectionData.highlights.programs.has(program.program_id)}
                    ...
                    <div className="w-px bg-gray-800 h-full flex-shrink-0"></div>

                    <div className="w-1/3 flex-shrink-0 flex flex-col h-full">
