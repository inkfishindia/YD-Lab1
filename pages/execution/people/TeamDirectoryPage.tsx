...
const TeamDirectoryPage: React.FC = () => {
  const { people, addPerson, updatePerson, deletePerson, roles } = useData();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingPerson, setEditingPerson] = useState<Person | null>(null);
  const [filters, setFilters] = useState({ name: '', department: '' });
  const [sortConfig, setSortConfig] = useState<SortConfig>(null);
  const { hasPermission } = usePermissions();
  const canWrite = hasPermission('people:write');
  ...
  return (
    <div className="container mx-auto">
        ...
        <table className="min-w-full divide-y divide-gray-700">
          <thead className="bg-gray-800">
            <tr>
              ...
              <TableHeader sortKey="role_title" label="Role Title" />
              <TableHeader sortKey="role_name" label="App Role" />
      ...
      <PersonFormModal isOpen={isModalOpen} onClose={closeModal} onSave={handleSave} person={editingPerson} roles={roles} />
    </div>
  );
};

const PersonFormModal: React.FC<{isOpen: boolean, onClose: () => void, onSave: (data: any) => void, person: Person | null, roles: Role[]}> = ({ isOpen, onClose, onSave, person, roles }) => {
    ...
    React.useEffect(() => {
      if (isOpen) {
        setFormData({
            ...
            role_name: person?.role_name || (roles[0]?.role_name || ''),
