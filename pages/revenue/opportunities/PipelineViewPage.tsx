...
export const PipelineViewPage: React.FC = () => {
  ...
  const sortedOpportunities = useMemo(() => {
    let sortableOpportunities = [...filteredOpportunities];
    if (sortConfig !== null) {
  ...
  const handleSave = (opportunityData: Omit<Opportunity, 'opportunity_id'> | Opportunity) => {
    if ('opportunity_id' in opportunityData) updateOpportunity(opportunityData);
    else addOpportunity(opportunityData);
    closeModal();
  };

  const TableHeader: React.FC<{ sortKey: keyof Opportunity, label: string }> = ({ sortKey, label }) => (
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-semibold text-white">Opportunity Pipeline</h1>
        <Button onClick={() => openModal()} className="flex items-center gap-2">
          <PlusIcon className="w-5 h-5" /> Add Opportunity
        </Button>
      </div>
      ...
      <div className="mb-4 p-4 bg-gray-900 border border-gray-800 rounded-lg flex items-center gap-4">
          ...
          <select name="stage" value={filters.stage} onChange={handleFilterChange} className="bg-gray-800 border border-gray-700 text-white rounded-md py-2 px-4 w-1/4 focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg shadow-md overflow-hidden">
        <table className="min-w-full divide-y divide-gray-700">
          <thead className="bg-gray-800">
            <tr>
              <TableHeader sortKey="opportunity_name" label="Opportunity Name" />
              <TableHeader sortKey="account_id" label="Account" />
          ...
          <tbody className="divide-y divide-gray-700">
            {sortedOpportunities.map((opportunity) => (
              <tr key={opportunity.opportunity_id} className="hover:bg-gray-800">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-white">{opportunity.opportunity_name}</div>
                  <div className="text-sm text-gray-400">{getAccountName(opportunity.account_id)}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{getAccountName(opportunity.account_id)}</td>
                <td className="px-6 py-4 whitespace-nowrap"><Badge text={opportunity.stage} colorClass={OPPORTUNITY_STAGE_COLORS[opportunity.stage]} /></td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">${(opportunity.amount || 0).toLocaleString()}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{opportunity.close_date}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{getPersonName(opportunity.owner_user_id)}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-4">
                  <button onClick={() => openModal(opportunity)} className="text-blue-400 hover:text-blue-300"><EditIcon className="w-5 h-5"/></button>
                  <button onClick={() => deleteOpportunity(opportunity.opportunity_id)} className="text-red-400 hover:text-red-300"><TrashIcon className="w-5 h-5"/></button>
                </td>
              </tr>
            ))}
          </tbody>
    ...
    React.useEffect(() => {
        if (isOpen) {
            if (opportunity) {
                ...
                setFormData({ ...getInitialOpportunityData(people, accounts), ...editableData });
            } else {
