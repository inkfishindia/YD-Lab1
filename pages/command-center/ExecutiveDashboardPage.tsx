...
const ExecutiveDashboardPage: React.FC = () => {
    ...
    const renderColumn = <T extends { [key: string]: any }>(
        ...
        <div className="flex flex-col bg-gray-900 border border-gray-800 rounded-lg">
            <div className="flex justify-between items-center p-4 border-b border-gray-800">
                ...
                <Button onClick={onAdd} disabled={true} variant="secondary" className="!p-2" title="This is a legacy view. Use the Execution page to add new items.">
                ...
                {items.length > 0 ? items.map(item => (
                    ...
                    >
                        ...
                        <div className="absolute top-2 right-2 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={(e) => { e.stopPropagation(); onEdit(item); }} disabled={true} title="This is a legacy view. Use the Execution page to edit items." className="p-1 text-blue-400 hover:text-blue-300 bg-gray-900/50 rounded disabled:opacity-50 disabled:cursor-not-allowed"><EditIcon className="w-4 h-4" /></button>
                            <button onClick={(e) => { e.stopPropagation(); onDelete(item[idKey]); }} disabled={true} title="This is a legacy view. Use the Execution page to delete items." className="p-1 text-red-400 hover:text-red-300 bg-gray-900/50 rounded disabled:opacity-50 disabled:cursor-not-allowed"><TrashIcon className="w-4 h-4" /></button>
                        </div>
                    </div>
    ...
    return (
        <div className="h-full flex flex-col">
            <h1 className="text-2xl font-semibold text-white mb-4">Executive Dashboard (Legacy)</h1>
            <p className="text-gray-400 mb-4 -mt-2 text-sm">This is a read-only view. For full CRUD functionality, please use the Execution page. Click an item to drill down.</p>
